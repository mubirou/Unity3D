...
Android スマホ + Unity エディタ + AR Core（実際には [AR Foundation]((https://unity.com/ja/unity/features/arfoundation)) を利用）による AR コンテンツ制作超入門  
...
1. プロジェクトの作成
    ...
    📝 AR テンプレートを利用すると [Package Manager]（Packages:Unity Registry）で [**AR Foundation**](https://unity.com/ja/unity/features/arfoundation) のインストール等が不要になります
